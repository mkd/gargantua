...
int UCI::parseMove(string str) {
  ...
  cout << "Move " << str << " not found. Generated moves: ";
  for (int move_count = 0; move_count < MoveList.count; move_count++) {
    cout << moveToString(MoveList.moves[move_count]) << " ";
  }
  cout << endl;
...
#undef NNUE_DO
#define NNUE_DO(m) Stockfish::Incremental::setup_move(m);

#undef NNUE_UNDO
#define NNUE_UNDO(m) Stockfish::Incremental::setup_undo(m);
#endif
...
void UCI::traceEval() {
  ...
  cout << showpos << fixed << setprecision(2)
       << "NNUE evaluation: " << evaluate() / 100.0f << endl
       << endl
       << flush;
...
// In addition to the UCI ones, also some additional debug commands are
// supported.
