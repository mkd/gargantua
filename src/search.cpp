...
std::array<int, 13> PieceValues = {100,           300,           300, 500, 900,
                                   ValueInfinite, 100,           300, 300, 500,
                                   900,           ValueInfinite, 0};
...
int negamax(int alpha, int beta, int depth, int excludedMove = 0) {
  ...
  if (ply &&
      ((score = TT::probe(alpha, beta, bestmove, depth)) != no_hash_found) &&
      !pv_node)
    if (fifty < 90)
      return score;
  ...
  if (ply && (countBits(occupancies[Both]) <= TB_LARGEST) && !castle) {
    ...
    if (sigsetjmp(syzygy_jmp_buf, 1) == 0) {
      wdl =
          tb_probe_wdl(occupancies[White], occupancies[Black],
  ...
  if (ply && !pv_node && (depth < 2) && ((StaticEval + RazorMargin) <= alpha)) {
    return qsearch(alpha, beta);
  }
  ...
  if (!pv_node && (countBits(occupancies[Both]) > 6)) {
...
moves_loop:
  ...
  for (int count = 0; count < MoveList.count; count++) {
    ...
    bool givesCheck = isSquareAttacked(
        (sideToMove == White) ? ls1b(bitboards[K]) : ls1b(bitboards[k]),
        sideToMove ^ 1);
    ...
    if (moves_searched == 0)
      ...
      if (ply && !pv_node && (depth <= 3) && !inCheck &&
          !getMoveCapture(MoveList.moves[count]) &&
...
void search() {
  ...
  ///////////////////////////////////////////////////////////////////////////
  //
  // Step 0. Syzygy Root Probe (DTZ)
  //
  // If the position is in the Syzygy tablebases (and we have enough pieces on
  // board to probe), we can query the DTZ table to get the absolute best move
  // that minimizes the distance to zero (mate or conversion).
  //
  // This solves the issue of "shuffling" in winning endgames (like KRvK) where
  // WDL probing alone says all moves are winning but doesn't distinguish
  // progress.

  if ((countBits(occupancies[Both]) <= TB_LARGEST) && !castle) {
    unsigned results[TB_MAX_MOVES];
    unsigned root_res = tb_probe_root(
        occupancies[White], occupancies[Black], bitboards[K] | bitboards[k],
        bitboards[Q] | bitboards[q], bitboards[R] | bitboards[r],
        bitboards[B] | bitboards[b], bitboards[N] | bitboards[n],
        bitboards[P] | bitboards[p], (fifty == 0) ? 0 : fifty,
        (epsq == NoSq) ? 0 : epsq, sideToMove == White, results);

    if (root_res != TB_RESULT_FAILED) {
      int dtz = TB_GET_DTZ(root_res);
      int wdl = TB_GET_WDL(root_res);

      if (wdl != TB_DRAW) {
        // Construct the move from TB result parts
        int from = TB_GET_FROM(root_res);
        int to = TB_GET_TO(root_res);
        int promo = TB_GET_PROMOTES(root_res);
        int ep = TB_GET_EP(root_res);

        // We need to encode the move fully for Gargantua (piece, capture flag,
        // etc) But tb_probe_root gives us strict from/to. We can find the move
        // in a generated move list to get the full correct encoding.
        ...
        for (int i = 0; i < MoveList.count; i++) {
          if (getMoveSource(MoveList.moves[i]) == from &&
              getMoveTarget(MoveList.moves[i]) == to) {

            // Check promo if applicable
            if (promo && getPromo(MoveList.moves[i]) != promo)
              continue;
...
int see(int move) {
  ...
  Bitboard attadef = getAttackers(White, toSq, occupiedBB) |
                     getAttackers(Black, toSq, occupiedBB);
  Bitboard maxXray =
      occupiedBB & ~(bitboards[N] | bitboards[K] | bitboards[n] | bitboards[k]);
