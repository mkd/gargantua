...
#define encodeMove(fromSq, toSq, piece, promo, capture, double, ep, castling)  \
  (fromSq) | (toSq << 6) | (piece << 12) | (promo << 16) | (capture << 20) |   \
      (double << 21) | (ep << 22) | (castling << 23)
...
static inline void printMove(int move) { std::cout << prettyMove(move); }
...
constexpr bool isSquareAttacked(int square, int side) {
  ...
  // assert((square >= a8) && (square <= h1)); // removed assert to be safe in
  // debug

  //bool debug = (square == 13 && side == White); // f7 attacked by White
  bool debug = false;

  if (debug)
    std::cout << "Checking attacks on f7 by White..." << std::endl;

  // square attacked by White or Black pawns
  if ((side == White) && (PawnAttacks[Black][square] & bitboards[P])) {
    if (debug)
      std::cout << "f7 attacked by White Pawn" << std::endl;
    return true;
  }

  if ((side == Black) && (PawnAttacks[White][square] & bitboards[p])) {
    if (debug)
      std::cout << "f7 attacked by Black Pawn" << std::endl;
    return true;
  }
  ...
  if (KnightAttacks[square] & ((side == White) ? bitboards[N] : bitboards[n])) {
    if (debug)
      std::cout << "f7 attacked by Knight" << std::endl;
    return true;
  }
  ...
  if (getBishopAttacks(square, occupancies[Both]) &
    ...
    if (debug)
      std::cout << "f7 attacked by Bishop" << std::endl;
    return true;
  }
  ...
  if (getRookAttacks(square, occupancies[Both]) &
    ...
    if (debug)
      std::cout << "f7 attacked by Rook" << std::endl;
    return true;
  }
  ...
  if (getQueenAttacks(square, occupancies[Both]) &
    ...
    if (debug)
      std::cout << "f7 attacked by Queen" << std::endl;
    return true;
  }
  ...
  if (KingAttacks[square] & ((side == White) ? bitboards[K] : bitboards[k])) {
    if (debug)
      std::cout << "f7 attacked by King" << std::endl;
    return true;
  }

  if (debug)
    std::cout << "f7 NOT attacked" << std::endl;
...
static inline int makeMove(int move) {
  ...
  if (!isSquareAttacked((sideToMove == White) ? ls1b(bitboards[k])
                                              : ls1b(bitboards[K]),
                        sideToMove))
    return 1;
